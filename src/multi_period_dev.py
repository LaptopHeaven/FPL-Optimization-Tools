...
def connect():
    print(
        "This method is provided for people who want to automate login. However with 2FA changes, it is unlikely to \
            work, and I'd suggest using team.json method from now on as explained in README file."
    )
    base_folder = Path()
    ...
    headers = {
        ...
        "cookie": "pl_euconsent-v2=CPUn1YtPUn1YtFCABAENBRCsAP_AAH_AAAwIF5wAQF5gXnABAXmAAAAA.YAAAAAAAAAAA; \
pl_euconsent-v2-intent-confirmed=^{^%^22tcf^%^22:^[755^]^%^2C^%^22oob^%^22:^[^]^}; \
pl_oob-vendors=^{^}; datadome=\
q_f99WYn0dSxMnuNGw8TQuIf2YeVio7m7Yk-jVBaWf43sM2v86YYVH17srelKLrR6B9ynB5V2z7pNs~jG6VKc~4u3up4WBrCYD6rekwuM3cRG8\
.9XbLGXAE4Cz~BWyv",
    }
...
def get_my_data(session, team_id):
    ...
    d = r.json()
    base_folder = Path()
    with open(base_folder / "../data/team-live.json", "w") as outfile:
        json.dump(d, outfile)
    d["team_id"] = team_id
    return d
...
def prep_data(my_data, options):
    ...
    merged_data = merged_data[
        (merged_data["total_min"] >= xmin_lb) | (merged_data["review_id"].isin(initial_squad))
    ].copy()
    ...
    if ev_per_price_cutoff != 0:
        ...
        merged_data = merged_data[
            (merged_data["total_ev"] / merged_data["now_cost"] > cutoff) | (merged_data["review_id"].isin(safe_players))
        ].copy()
...
def solve_multi_period_fpl(data, options):
    ...
    # Data
    problem_name = (
        f"mp_h{horizon}_regular" if objective == "regular" else f"mp_h{horizon}_o{objective[0]}_d{decay_base}"
    )
    ...
    # Dictionaries
    lineup_type_count = {
        (t, w): so.expr_sum(lineup[p, w] for p in players if merged_data.loc[p, "element_type"] == t)
        for t in element_types
        for w in gameweeks
    }
    squad_type_count = {
        (t, w): so.expr_sum(squad[p, w] for p in players if merged_data.loc[p, "element_type"] == t)
        for t in element_types
        for w in gameweeks
    }
    squad_fh_type_count = {
        (t, w): so.expr_sum(squad_fh[p, w] for p in players if merged_data.loc[p, "element_type"] == t)
        for t in element_types
        for w in gameweeks
    }
    ...
    model.add_constraints(
        (squad_fh_count[w] == 15 * use_fh[w] for w in gameweeks),
        name="squad_fh_count",
    )
    model.add_constraints(
        (so.expr_sum(lineup[p, w] for p in players) == 11 + 4 * use_bb[w] for w in gameweeks),
        name="lineup_count",
    )
    model.add_constraints(
    ...
    model.add_constraints(
        (lineup[p, w] <= squad_fh[p, w] + 1 - use_fh[w] for p in players for w in gameweeks),
    ...
    model.add_constraints(
        (
            so.expr_sum(squad[p, w] for p in players if merged_data.loc[p, "name"] == t) <= 3
            for t in teams
            for w in gameweeks
        ),
        name="team_limit",
    ...
    model.add_constraints(
        (
            so.expr_sum(squad_fh[p, w] for p in players if merged_data.loc[p, "name"] == t) <= 3 * use_fh[w]
            for t in teams
            for w in gameweeks
        ),
        name="team_limit_fh",
    ...
    model.add_constraints(
        (in_the_bank[w] == in_the_bank[w - 1] + sold_amount[w] - bought_amount[w] for w in gameweeks),
        name="cont_budget",
    )
    model.add_constraints(
        (
    ...
    model.add_constraints(
        (
            free_transfers[w - 1] - number_of_transfers[w - 1] - 2 * use_wc[w - 1] - 2 * use_fh[w - 1] <= 2 * aux[w]
            for w in gameweeks
            if w > threshold_gw
        ),
        name="force_aux_1",
    ...
    model.add_constraints(
        (
            free_transfers[w - 1] - number_of_transfers[w - 1] - 2 * use_wc[w - 1] - 2 * use_fh[w - 1]
            >= aux[w] + (-14) * (1 - aux[w])
            for w in gameweeks
            if w > threshold_gw
        ),
    ...
    ## Chip constraints
    model.add_constraints(
        (use_wc[w] + use_fh[w] + use_bb[w] <= 1 for w in gameweeks),
        name="single_chip",
    )
    model.add_constraints(
    ...
    ## Transfer in/out fix
    model.add_constraints(
        (transfer_in[p, w] + transfer_out[p, w] <= 1 for p in players for w in gameweeks),
    ...
    if options.get("locked", None) is not None:
        ...
        model.add_constraints(
            (squad[p, w] == 1 for p in locked_players for w in gameweeks),
            name="lock_player",
        )
    ...
    if options.get("no_future_transfer"):
        model.add_constraint(
            so.expr_sum(
                transfer_in[p, w] for p in players for w in gameweeks if w > next_gw and w != options.get("use_wc")
            )
            == 0,
    ...
    if options.get("num_transfers", None) is not None:
        model.add_constraint(
            so.expr_sum(transfer_in[p, next_gw] for p in players) == options["num_transfers"],
            name="tr_limit",
        )

    if options.get("hit_limit", None) is not None:
        model.add_constraint(
            so.expr_sum(penalized_transfers[w] for w in gameweeks) <= options["hit_limit"],
    ...
    if options.get("no_opposing_play") is True:
        for gw in gameweeks:
            ...
            model.add_constraints(
                (lineup[p1, gw] + lineup[p2, gw] <= 1 for (p1, p2) in opposing_players),
                name=f"no_opp_{gw}",
            )
    ...
    gw_total = {
        w: gw_xp[w] - 4 * penalized_transfers[w] + ft_value * free_transfers[w] + itb_value * in_the_bank[w]
        for w in gameweeks
    }
    ...
    for iter in range(iteration):
                ...
                if squad[p, w].get_value() + squad_fh[p, w].get_value() + transfer_out[p, w].get_value() > 0.5:
                    ...
                    player_sell_price = (
                        ...
                        if not is_transfer_out
                            ...
                            sell_price[p]
                            if p in price_modified_players and transfer_out_first[p, w].get_value() > 0.5
                            else buy_price[p]
                        )
                    )
        ...
        picks_df = pd.DataFrame(
            ...
            by=["week", "lineup", "type", "xP"],
            ascending=[True, False, True, True],
        )
        total_xp = so.expr_sum(
            (lineup[p, w] + captain[p, w]) * points_player_week[p, w] for p in players for w in gameweeks
        ).get_value()
        ...
        if iteration_criteria == "this_gw_transfer_in":
            actions = so.expr_sum(
                1 - transfer_in[p, next_gw] for p in players if transfer_in[p, next_gw].get_value() > 0.5
            ...
            actions = so.expr_sum(
                1 - transfer_out[p, next_gw] for p in players if transfer_out[p, next_gw].get_value() > 0.5
